...
public:
	...
	ICBSSearch(const MapLoader& ml, const AgentsLoader& al, double f_w, 
		heuristics_type h_type, bool PC, bool rootOnly, bool cardinalConstraint, 
		bool EPEA4PAIR, int CGSolver, int nodesThreshold, bool booking, int time_limit, int node_limit, int screen, bool log);
	ICBSSearch(vector<SingleAgentICBS*>& search_engines, const vector<list<Constraint>>& constraints,
		vector<vector<PathEntry>>& paths_found_initially, double f_w, 
		heuristics_type h_type, bool PC, int cost_upperbound, int time_limit, int screen);
...
private:
	...
	bool rootOnly; // run PAIR only at root
	bool cardinalConstraint; // add constraints at goal locations if the conflicts between two agents are cardinal
	...
	int getEdgeWeight(int a1, int a2, const vector<list<Constraint>> & constraints, ICBSNode& node, bool& hit);
